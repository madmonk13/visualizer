...
class MusicVisualizer:
    def __init__(self, audio_path, output_path='output.mp4', cover_image_path=None, 
        ...
        self.palettes = {
            'rainbow': {
                'name': 'Rainbow',
                'colors': [0, 45, 90, 135, 180, 225, 270, 315],  # Full spectrum
                'saturation': 1.0,
                'brightness': 1.0
            },
            'spring': {
                'name': 'Spring',
                'colors': [80, 100, 120, 140, 280, 300, 320, 340],  # Greens, blues, pinks
                'saturation': 0.8,
                'brightness': 0.95
            },
            'summer': {
                'name': 'Summer',
                'colors': [30, 45, 60, 180, 200, 220, 240, 260],  # Oranges, yellows, blues
                'saturation': 1.0,
                'brightness': 1.0
            },
            'autumn': {
                'name': 'Autumn',
                'colors': [0, 15, 30, 35, 40, 25, 20, 10],  # Reds, oranges, golds
                'saturation': 0.9,
                'brightness': 0.85
            },
            'winter': {
                'name': 'Winter',
                'colors': [180, 200, 220, 240, 260, 200, 190, 210],  # Blues, cyans, cool tones
                'saturation': 0.7,
                'brightness': 0.9
            }
        }
        ...
        # Text fade tracking
        self.text_fade_history = []  # Store recent volume values for smooth fading
    ...
    def draw_waveforms_with_glow(self, img, frame_idx):
        ...
        for band_idx, band in enumerate(self.bands):
            waveform = self.get_band_waveform(frame_idx, band_idx, points=150)
            ...
            for layer in range(2):
                ...
                if len(points_upper) > 1:
                    for thickness in range(12, 0, -1):
                        ...
                        glow_intensity = (1 - thickness / 12) * 0.5
        ...
        waveform_layer = waveform_layer.filter(ImageFilter.GaussianBlur(radius=2))
    ...
    def draw_cover_kaleidoscope(self, img, frame_idx, volume_intensity, beat_intensity):
        ...
        if self.cover_image:
            ...
            # Create kaleidoscope layer
            kaleidoscope = Image.new('RGBA', (self.width, self.height), (0, 0, 0, 0))
            
            # Draw kaleidoscope fragments WITHOUT rotation - STATIONARY
            num_fragments = 12
            for i in range(num_fragments):
                angle = (i * 2 * math.pi / num_fragments)
                
                # Get band value for this fragment
                band_idx = i % len(self.bands)
                band_value = normalized_bands[band_idx]
                distance = int(base_size * 0.3 + band_value * 50)
                
                # Calculate triangle points
                x1, y1 = center_x, center_y
                x2 = int(center_x + distance * math.cos(angle - math.pi / 12))
                y2 = int(center_y + distance * math.sin(angle - math.pi / 12))
                x3 = int(center_x + distance * math.cos(angle + math.pi / 12))
                y3 = int(center_y + distance * math.sin(angle + math.pi / 12))
                
                # Create fragment with color tint
                fragment = Image.new('RGBA', (self.width, self.height), (0, 0, 0, 0))
                fragment_draw = ImageDraw.Draw(fragment)
                fragment_draw.polygon([(x1, y1), (x2, y2), (x3, y3)], fill=(255, 255, 255, 180))
                
                # Apply cover
                cover_size = base_size * 2
                cover_resized = self.cover_image.resize((cover_size, cover_size))
                
                # Composite fragment
                temp = Image.new('RGBA', (self.width, self.height), (0, 0, 0, 0))
                temp.paste(cover_resized, (center_x - cover_size // 2, center_y - cover_size // 2))
                temp = Image.composite(temp, Image.new('RGBA', temp.size, (0, 0, 0, 0)), fragment)
                
                kaleidoscope = Image.alpha_composite(kaleidoscope, temp)
            
            # Apply blur for glow
            kaleidoscope_glow = kaleidoscope.filter(ImageFilter.GaussianBlur(radius=3))
            img.paste(kaleidoscope_glow, (0, 0), kaleidoscope_glow)
            
            # Draw center circle with pulsing
            center_size = int(base_size * 0.15 * (1 + volume_intensity * 0.5 + beat_intensity * 0.8))
        ...
        # Draw glowing rings (always drawn, with rotation applied if specified)
        ring_layer = Image.new('RGBA', (self.width, self.height), (0, 0, 0, 0))
        
        # Apply rotation to ring layer if specified
        if self.ring_rotation != 'z':
            # For X and Y axis rotation, we'll create a 3D-like perspective effect
        ...
        for r in range(3):
            ...
            if self.ring_rotation == 'x':
                # X-axis rotation (horizontal squeeze)
                distortion = 0.6 + 0.4 * math.cos(self.rotation * 2)
                ellipse_height = int(ring_size * distortion)
                ellipse_width = ring_size
            elif self.ring_rotation == 'y':
                # Y-axis rotation (vertical squeeze)
                distortion = 0.6 + 0.4 * math.cos(self.rotation * 2)
                ellipse_width = int(ring_size * distortion)
                ellipse_height = ring_size
            else:
                ellipse_width = ring_size
                ellipse_height = ring_size
            ...
            for glow in range(8, 0, -1):
                ...
                ring_draw.ellipse(
                    [center_x - ellipse_width - glow*2, center_y - ellipse_height - glow*2,
                     center_x + ellipse_width + glow*2, center_y + ellipse_height + glow*2],
    ...
    def render_frame(self, frame_idx, total_frames):
        ...
        # Draw starfield (behind everything) - NOT rotated
...
Features:
  ...
  - Multiple rotation axes for 3D effects
    ...
    parser.add_argument('--waveform-rotation', default='z',
                       choices=['x', 'y', 'z'],
                       help='Waveform rotation axis: x=horizontal flip, y=vertical tilt, z=standard spin (default: z)')
    
    parser.add_argument('--ring-rotation', default='z',
                       choices=['x', 'y', 'z'],
                       help='Ring rotation axis: x=horizontal squeeze, y=vertical squeeze, z=no rotation (default: z)')
